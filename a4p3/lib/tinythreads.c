...
void spawnWithDeadline(void (* function)(int), int arg, unsigned int deadline, unsigned int rel_deadline) {
	...
	newp->arg = arg;
	newp->next = NULL;
	newp->Period_Deadline = deadline; // added
	newp->Rel_Period_Deadline = rel_deadline; // added 
	if (setjmp(newp->context) == 1) {
		ENABLE();
		current->function(current->arg);
		DISABLE();
...
static void sortX(thread *queue) {
	...
	while (temp->next != NULL) {
		if (temp->Rel_Period_Deadline > temp->next->Rel_Period_Deadline){ 
...
void respawn_periodic_tasks(void) {
	...
	int i = 0;
	while (p) {	
		if (ticks % p->Period_Deadline == 0) { // changed 
			thread temp = p->next;
			...
			t->Rel_Period_Deadline = t->Period_Deadline + ticks;
			enqueue(t,&readyQ);
			p = temp;
		} else {
			i++;
